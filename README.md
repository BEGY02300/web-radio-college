...
## 🆕 Nouveautés (v4.0) — sécurité, modération, profils
- 🔐 **5 méthodes de connexion** : Email/Pseudo + mot de passe, 🔵 Google, 📱 Téléphone (SMS), 🐙 GitHub, 🟣 Yahoo (Apple/Game Center retiré)
- 🆔 **Connexion par pseudo OU email** + reset password par pseudo
- 👤 **Profils utilisateur** — pseudo unique, avatar (URL d'image), bio, liaison/délaison de plusieurs providers
- 🛡️ **Modération automatique** :
  - Anti-insultes (`merde`, `con`, `fdp`, `salope`, `enculé`, `ntm`, etc.) avec normalisation Unicode
  - Anti-phishing (whitelist `youtube.com`, `youtu.be`, `google.com`, `firebaseapp.com`)
  - Anti-spam : 500 caractères max, rate-limit 1 message / 5 s, 5 commentaires / minute
  - Sanitisation XSS (jamais de `dangerouslySetInnerHTML`)
  - Blocage images hors `png/jpg/jpeg/gif/webp/avif`
- 🚩 **Système de signalement** — bouton 🚩 sur chaque commentaire, masquage automatique à 3 signalements
- 🚫 **Bans admin** — onglet "Signalements" dans le panneau admin → bannir / débannir
- 🎧 **Lecteur audio Spotify-like** (cover, skip ±10/30s, gradient progress, volume)
- 📁 **Dossiers dépliants** — clic = déroule les médias dedans
- 🖼️ **Covers automatiques** — `episode01.mp3` + `episode01.webp` = cover affichée
- 🎥 **YouTube non répertorié** via fichiers `.url` / `.txt`
- 🌿 Fallback automatique de branche si `1` n'existe pas
- 📊 Logs console F12 enrichis
...
- 🔐 Connexion via 5 fournisseurs : Email/Pseudo, Google, SMS, GitHub, Yahoo
- 👤 Profils éditables (avatar, bio, pseudo unique)
- 🛡️ Modération automatique (anti-insulte, anti-phishing, anti-spam, anti-XSS)
- 🚩 Signalements + bans admin
...
### 🆕 v4.0 — Profil & Modération

**Profil utilisateur** (clic sur l'avatar dans le header) :
- ✏️ Modifier son **pseudo** (3-20 caractères, alphanumérique + `._-`, unique global)
- 🖼️ **Avatar** par URL d'image (png, jpg, jpeg, gif, webp, avif uniquement)
- 📝 **Bio** (300 caractères max, modération automatique)
- 🔗 **Lier/délier** Google, GitHub, Yahoo, Téléphone, Email + mot de passe à son compte (un seul UID, plusieurs façons de se connecter)

**Modération** (`js/moderation.js`) — toutes les fonctions sont activées automatiquement :

| Fonction                       | Effet                                                                  |
| ------------------------------ | ---------------------------------------------------------------------- |
| `checkBadWords(text)`          | Détecte insultes (normalisation Unicode, leetspeak partiel)            |
| `isSafeUrl(url)`               | Whitelist `youtube.com`, `youtu.be`, `google.com`, `firebaseapp.com`   |
| `checkSuspiciousLinks(text)`   | Refuse tout lien hors whitelist                                        |
| `checkRateLimit(userId)`       | 1 message / 5 s + 5 messages / minute par utilisateur                  |
| `sanitizeText(text, maxLen)`   | Échappe HTML/JS, limite la longueur                                    |
| `isAllowedImageUrl(url)`       | N'accepte que `png, jpg, jpeg, gif, webp, avif`                        |
| `validateUsername(name)`       | Forme + bad words                                                      |
| `validateBio(text)`            | Longueur + bad words + liens suspects                                  |

**Signalements** :
- Chaque utilisateur peut **signaler** un commentaire (bouton 🚩) avec un motif
- À **3 signalements**, le commentaire est **masqué automatiquement** (un bouton "Afficher quand même" reste accessible côté lecteur)
- Les admins voient un onglet **"Signalements"** dans le panneau admin avec : auteurs, motifs, action **Bannir** / **Débannir**

**Bans** :
- Un utilisateur banni voit une bannière rouge en haut du site
- Il peut continuer à naviguer mais **ne peut plus poster** (vérification serveur côté `comments.js`)
- Levée de ban en un clic depuis le panneau admin
