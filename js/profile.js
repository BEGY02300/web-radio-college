...
export async function upsertProfile(uid, fields = {}) {
  ...
  if (fields.avatar_url !== undefined) {
    if (fields.avatar_url) {
      // Accepter : Supabase Storage (notre bucket) OU domaines whitelist OU data:image (preview)
      const isSupabase = fields.avatar_url.includes('/storage/v1/object/public/');
      const isData = fields.avatar_url.startsWith('data:image/');
      if (!isSupabase && !isData && !isAllowedImageUrl(fields.avatar_url)) {
...
export async function syncProfileFromUser(user) {
  ...
  if (!existing) {
    ...
    // Avatar par défaut : photoURL si domaine sûr, sinon null
    const initialAvatar = user.photoURL && (
      user.photoURL.includes('googleusercontent.com') ||
      user.photoURL.includes('githubusercontent.com') ||
      isAllowedImageUrl(user.photoURL)
    ) ? user.photoURL : null;
  ...
  // Sinon, on met juste à jour les champs techniques (mais on garde l'avatar choisi par l'utilisateur)
  return upsertProfile(user.uid, {
...
export async function uploadAvatar(uid, file) {
  ...
  if (!ALLOWED_AVATAR_TYPES.includes(file.type)) {
    throw new Error(`Format non supporté. Utilisez : PNG, JPEG, WebP, GIF`);
  }
  if (file.size > MAX_AVATAR_SIZE) {
    throw new Error(`Fichier trop lourd (max 2 Mo). Taille actuelle : ${(file.size / 1024 / 1024).toFixed(1)} Mo`);
  }

  const ext = file.name.split('.').pop().toLowerCase();
  const fileName = `${uid}/avatar-${Date.now()}.${ext}`;

  logger.info(`📤 Upload avatar : ${fileName} (${(file.size / 1024).toFixed(1)} Ko)`);

  const bucket = SUPABASE_CONFIG.avatarBucket || 'avatars';
  ...
  if (upErr) {
    logger.error('Upload avatar échoué', { err: upErr.message });
    if (upErr.message?.includes('Bucket not found')) {
      throw new Error('Bucket "avatars" introuvable. Exécutez supabase-setup.sql.');
    }
    throw upErr;
  }
  ...
  logger.success('✓ Avatar uploadé', { url: data.publicUrl });
  return data.publicUrl;
}

/**
 * Supprime l'avatar courant du storage (best effort).
 */
export async function deleteAvatar(avatarUrl) {
  ...
  try {
    ...
    const m = avatarUrl.match(/\/storage\/v1\/object\/public\/[^/]+\/(.+)$/);
    if (!m) return;
    await supabase.storage.from(bucket).remove([m[1]]);
    logger.info('🗑️ Ancien avatar supprimé');
  } catch (e) {
    logger.warn('Échec suppression ancien avatar', { err: e.message });
  }
}
